defmodule OpenJTalk.InfoTest do
  ...
  test "info/0 returns a well-formed map" do
    ...
    assert is_binary(bin_path) and bin_path != ""
    assert bin_src in [:env, :bundled, :system]

    assert is_binary(dic_path) and dic_path != ""
    assert dic_src in [:env, :bundled, :system]

    assert is_binary(voice_path) and voice_path != ""
    assert voice_src in [:env, :bundled, :system]
